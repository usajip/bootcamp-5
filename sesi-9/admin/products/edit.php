...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validation
    if (empty($nama_produk) || empty($kategori) || $harga <= 0 || $stok < 0 || empty($deskripsi)) {
        $error_message = "Nama produk, kategori, harga, stok, dan deskripsi harus diisi dengan benar!";
    } else {
        // Handle file upload
        $gambar = $product['image']; // Keep existing image by default
        ...
        if (empty($error_message)) {
            $sql = "UPDATE product SET name = ?, category = ?, price = ?, image = ?, description = ?, stock = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssissii", $nama_produk, $kategori, $harga, $gambar, $deskripsi, $stok, $product_id);
...
<head>
    ...
    <title>Edit Produk | Admin EduShop</title>
    ...
    <div class="main-content">
        ...
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">
                    <i class="fas fa-edit me-2"></i>Edit Produk: <?= htmlspecialchars($product['name']) ?>
                </h5>
            </div>
            <div class="card-body">
                        ...
                        <div class="col-md-8">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" 
                                          ...
                                          name="deskripsi" 
                                          rows="4" 
                                          placeholder="Deskripsi produk (opsional)"><?= htmlspecialchars($product['description']) ?></textarea>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <!-- <div class="drop-zone" id="dropZone">
                                ...
                                <div id="dropContent">
                                    ...
                                    <p class="mb-2">Klik untuk ubah gambar atau drag & drop</p>
    ...
    <script>
        ...
        document.getElementById('newCategoryBtn').addEventListener('click', function() {
            ...
            if (newInput.classList.contains('d-none')) {
                ...
                select.value = '<?= htmlspecialchars($product['kategori']) ?>';
            }
        });
        ...
        document.querySelector('button[type="reset"]').addEventListener('click', function(e) {
            ...
            document.getElementById('nama_produk').value = '<?= htmlspecialchars($product['nama_produk']) ?>';
            document.getElementById('kategori').value = '<?= htmlspecialchars($product['kategori']) ?>';
            document.getElementById('harga').value = '<?= $product['harga'] ?>';
            document.getElementById('stok').value = '<?= $product['stok'] ?>';
            document.getElementById('deskripsi').value = '<?= htmlspecialchars($product['deskripsi']) ?>';
            ...
            document.getElementById('new_kategori').classList.add('d-none');
            document.getElementById('new_kategori').value = '';
            document.getElementById('newCategoryBtn').innerHTML = '<i class="fas fa-plus"></i>';
        });
