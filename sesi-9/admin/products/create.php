...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_produk) || empty($kategori) || $harga <= 0 || $stok < 0 || empty($deskripsi)) {
        ...
        if (empty($error_message)) {
            $sql = "INSERT INTO product (name, category, price, image, description, stock) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssissi", $nama_produk, $kategori, $harga, $gambar, $deskripsi, $stok);
                        ...
                        <div class="col-md-8">
                                            ...
                                            <select class="form-select" id="kategori" name="kategori" required>
                                                ...
                                                while($category = $categories_result->fetch_assoc()): ?>
                                                    <option value="<?= htmlspecialchars($category['category']) ?>"
                                                            <?= (isset($_POST['kategori']) && $_POST['kategori'] === $category['category']) ? 'selected' : '' ?>>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" 
                                          ...
                                          name="deskripsi" 
                                          rows="4" 
                                          placeholder="Deskripsi produk" required><?= isset($_POST['deskripsi']) ? htmlspecialchars($_POST['deskripsi']) : '' ?></textarea>
                            </div>
                        </div>
